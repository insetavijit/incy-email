var
    /* | Gulp Handler Plugins  | */
    gulp = require("gulp"),
    colors = require("ansi-colors"),
    // utils :
    $ = require("gulp-load-plugins")(),
    pump = require("pump"),
    /*| project info handlers| */
    vl = require("./vl.js"), // FOLDER STUCTURE LIST
    pkg = require("./package.json"), //PROJECT INFO
    fs = require('fs'), //NODE : FILE SYSTEM 
    args = getCmnds(process.argv.slice(2)), //GRAB OPTIONAL PARAMITERS (--PARAM)
    // glob = require("glob"),
    projectName = pkg.name, //NAME OF THE PROJECT ( DRIVEN FORM "./package.json")
    /*| OUTPUT | */
    /*| system tools |*/
    del = require("del") //DELETING FILES 
;

// out put a wellcome msg . and some status
gulp.task("hi",
    gulp.parallel(wellcome));
// clears ( remove content of ) all listed dirs with paramiters :
gulp.task("clear",
    gulp.parallel(clr));
// clears ( remove content of ) .bin dir
gulp.task("clr:bin", gulp.parallel((done) => {
    clr(done, [vl.base.bin])
}));
// clears ( remove content of ) dist dir
gulp.task("clr:dist", gulp.parallel((done) => {
    clr(done, [vl.base.dist])
}));
// clears ( remove content of ) all listed dirs ( see inside code )
gulp.task("clr:all", gulp.parallel((done) => {
    clr(done, [vl.base.dist, vl.base.bin])
}));
//productivity apps :
gulp.task("to:jade", gulp.parallel((done)=>{
    pump([
        gulp.src(vl.tools.html2jade.src),
        $.html2jade({
            tabs:true,
            bodyless:true
        }),
        gulp.dest(vl.tools.html2jade.dist)
    ], done() )
}));

/**
 * @param {*} done 
 * @param {Array} param 
 * @version 1.0
 * created on : 2018-07-24 01:33:54 AM GMT+0530 (India Standard Time)
 */
function clr(done, dirListToClr = []) {
    //if args is not empty then clear dirs mentiond in args
    if (dirListToClr.length === 0) {
        //NO PARAM IS FOUND : we will set -df global params as clr:param
        dirListToClr = args;
    }

    // clearing the directorys
    dirListToClr.forEach(clrDirs => {
        del(clrDirs + "/*");
    });
    // pass the complete signal
    return done();
}
/**
 * @param {Array} param 
 * @description :
 *      collects all cmnds / paramiters starting with ( "--") .
 *      no snyc signal is passed.
 * @returns {Array} all cmnds
 * @version 1.0
 * created on : 2018-07-24 01:33:54 AM GMT+0530 (India Standard Time)
 */
function getCmnds(param) {
    var cmnds = [];
    param.forEach(arg => {
        (arg.split("--").length > 1) ? cmnds.push(arg.split("--").pop()): false;
    });
    return cmnds
}
function wellcome(done) {
    console.log("HI")
    done();
}